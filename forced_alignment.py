...
def ffmpeg_split(
    audio_file: Path, begin: str, end: str, output_audio_path: Path
) -> None:
    # fmt: off
    subprocess.run(
        [
...
def split_audio_preserving_chapters(
    fragments: Any, texts_lines: TextsLines
) -> list[TimedText]:
    ...
    for title, text_lines in texts_lines:
        ...
        logging.debug(
            "(Fragments {:{width}} - {:{width}}) "
            "{start_time} - {end_time} ({interval}s)".format(
                fragment_idxs[0],
                fragment_idxs[-1],
        ...
        if cur_interval == 0 and timed_text.text:
            ...
            ctx = (
                f"{timed_text.text[:200]}{'...' if len(timed_text.text) > 200 else ''}"
            )
...
def write_timed_texts(
    timed_texts: list[TimedText], audio_path: Path, output_dir: Path
) -> None:
...
def get_sorted_file_paths(files_dir: Path, extension: str) -> list[Path]:
    ...
    file_paths = [
        ...
        if fp.suffix == extension and "merged" not in fp.name
    ]
...
def merge_audios(merged_audio_path: Path) -> None:
    ...
    if not merged_audio_path.exists():
        logging.error(
            f"[merge_audio_files] Could not find the merged file: {merged_audio_path}"
        )
        exit(1)
...
def run(
    ...
    if n_sections > 0:
        ...
        logging.debug(
            f"Given {n_sections=}, the new cut_at_seconds is {cut_at_seconds}"
        )
